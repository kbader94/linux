...
static int tx_trig_to_fcr(struct uart_8250_port *up, u32 level)
{
	...
	for (int i = 0; i < UART_8250_FIFO_TRIG_MAX_STATE; i++) {
		if (conf->txtrig_bytes[i] && conf->txtrig_bytes[i] == level)
			return UART_8250_FCR_T_FROM_TRIG_I(i);
	}
...
/*
 * Search a per-part trigger-level table for the best match per @round:
 *   ROUND_EXACT - the entry equals @want, else -ERANGE
 *   ROUND_DOWN  - the largest entry <= @want, else -ERANGE
 *   ROUND_UP    - the smallest entry >= @want, else -ERANGE
 *
 * The table is not assumed to be sorted (some parts pack programmer-order
 * into the index); zero entries are skipped so packed/short tables work.
 * Returns the matched level in *@out on success.
 */
static int serial8250_round_trig(const u8 *table, size_t n, u32 want,
				 enum uart_fifo_round round, u32 *out)
{
	bool found = false;
	u32 best = 0;
	size_t i;
	...
	if (round == UART_FIFO_ROUND_EXACT || !found)
		return -ERANGE;
 ...
 *
 * Programs FCR with the requested trigger / enable bits and updates the
 * cached up->fcr.
 *
 * If a future caller does set CLEAR_RCVR / CLEAR_XMIT in @fcr the write
 * to hardware honours them (the chip performs the clear), but those bits
 * are masked out of @up->fcr so subsequent uses of up->fcr do not re-apply
 * the clear.
 */
					 ...
					 enum uart_fifo_round round)
{
	...
	u32 rounded_rx = ctl->rx_trigger_bytes;
	u32 rounded_tx = ctl->tx_trigger_bytes;
	int fcr_rx_trig, fcr_tx_trig, ret;
	u8 save_lcr, efr;

	/* Enable EFR[4] - Required to set Tx FIFO level */
	save_lcr = serial_in(up, UART_LCR);
	/* Must set LCR = 0xBF to enable EFR[4] */
	serial_out(up, UART_LCR, UART_LCR_CONF_MODE_B); /* LCR = 0xBF */
	efr = serial_in(up, UART_EFR);
	serial_out(up, UART_EFR, efr | UART_EFR_ECB);
	serial_out(up, UART_LCR, save_lcr);

	ret = serial8250_round_trig(cfg->rxtrig_bytes,
				    UART_8250_FIFO_TRIG_MAX_STATE,
				    rounded_rx, round, &rounded_rx);
	if (ret)
		return ret;
	ctl->rx_trigger_bytes = rounded_rx;

	ret = serial8250_round_trig(cfg->txtrig_bytes,
				    UART_8250_FIFO_TRIG_MAX_STATE,
				    rounded_tx, round, &rounded_tx);
	if (ret)
		return ret;
	ctl->tx_trigger_bytes = rounded_tx;

	fcr_rx_trig = rx_trig_to_fcr(up, rounded_rx);
	if (fcr_rx_trig < 0)
		return fcr_rx_trig;

	fcr_tx_trig = tx_trig_to_fcr(up, rounded_tx);
	if (fcr_tx_trig < 0)
		return fcr_tx_trig;

	/* 'DMA' mode is required for programmable FIFO levels.
	 * Note: DMA is a legacy term here and does NOT mean Direct Memory Access
	 * See section 2.5 of ST16C650A datasheet
	 */
...
{
	const struct serial8250_config *cfg = &uart_config[up->port.type];
	u32 rounded = ctl->rx_trigger_bytes;
	int fcr_rx_trig, ret;

	/* TX fifo levels not supported on 16550A */
	...
	if (ret)
		return ret;
	ctl->rx_trigger_bytes = rounded;

	fcr_rx_trig = rx_trig_to_fcr(up, rounded);
	if (fcr_rx_trig < 0)
		return fcr_rx_trig;
...
static const struct serial8250_config uart_config[] = {
	...
	[PORT_16550A] = {
		...
		.set_fifo_control = port_16550A_set_fifo_control,
	},
	[PORT_CIRRUS] = {
		.name		= "Cirrus",
		.fifo_size	= 1,
	...
	[PORT_16650V2] = {
		...
		.flags		= UART_CAP_FIFO | UART_CAP_EFR | UART_CAP_SLEEP,
		.set_fifo_control = port_16650V2_set_fifo_control,
	},
	[PORT_16750] = {
		.name		= "TI16750",
		.fifo_size	= 64,
				       ...
				       enum uart_fifo_round round)
{
	...
	const struct serial8250_config *cfg;
	struct uart_fifo_control local;
	unsigned long flags;
	int ret;

	if (WARN_ON_ONCE(port->type <= PORT_UNKNOWN ||
			 port->type >= ARRAY_SIZE(uart_config)))
		return -EINVAL;

	if (!(up->capabilities & UART_CAP_FIFO))
		return -EOPNOTSUPP;

	cfg = &uart_config[port->type];
	if (!cfg->set_fifo_control)
		return -EOPNOTSUPP;

	/* Writable copy so the per-part callback can report back the rounded
	 * trigger levels it actually programmed; we then cache those.
	 */
	local = *ctl;
	uart_port_lock_irqsave(&up->port, &flags);
	ret = cfg->set_fifo_control(up, &local, round);
	if (!ret)
		up->fifo_control = local;
...
static void serial8250_config_port(struct uart_port *port, int flags)
{
	...
	up->fifo_control = uart_config[up->port.type].fifo_control;
