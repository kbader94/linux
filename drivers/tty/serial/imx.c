				...
				const struct uart_fifo_control *ctl)
{
	...
	struct imx_port *sport = to_imx_port(port);
	struct device *dev = sport->port.dev;
	unsigned char txwl, rxwl;
	unsigned long flags;
	unsigned int rx_maxburst;

	txwl = ctl->tx_trigger_bytes;
	rxwl = ctl->rx_trigger_bytes;

	if (!(ctl->flags & UART_FIFO_CTRL_FLAG_ENABLE_FIFO))
		return -EOPNOTSUPP; /* IMX does not support disabling FIFO */

	if (txwl < 1 || txwl > port->fifosize ||
	    rxwl < 1 || rxwl > port->fifosize)
		return -EINVAL;
				...
				struct uart_fifo_control *ctl)
{
	...
	ctl->rx_trigger_bytes = (ufcr >> UFCR_RXTL_SHF) & UFCR_RXTL_MASK; /* == ufcr & 0x3f */
	ctl->tx_trigger_bytes = (ufcr >> UFCR_TXTL_SHF) & UFCR_RXTL_MASK;

	/* i.MX FIFO can’t be disabled; report it as enabled. */
