 ...
 * @resp_id:    frame ID whose registered publisher supplied the
 *              response data; only consulted when @emit_flags
 *              includes LIN_EMIT_PUBLISHER. For unconditional and
 *              sporadic slots this equals frame->lin_id, but an
 *              event response is carried under the trigger ID while
 *              its data comes from the answering frame's publisher,
 *              so the answering ID is what locates the owning sock.
 *
